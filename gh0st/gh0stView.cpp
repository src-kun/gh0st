...
void CGh0stView::OnInitialUpdate()
{
	...
	m_pListCtrl->SetExtendedStyle(LVIF_PARAM | LVS_EX_FULLROWSELECT /*| LVS_EX_FLATSB*/ |       //有复选框
		...
		//	LVS_EX_ONECLICKACTIVATE | LVS_EX_UNDERLINEHOT |LVS_EX_SUBITEMIMAGES| LVS_EX_GRIDLINES);     //网格线
		LVS_EX_ONECLICKACTIVATE | LVS_EX_UNDERLINEHOT |LVS_EX_SUBITEMIMAGES/*| LVS_EX_GRIDLINES*/);  //无网格线
	
	// 改变在线主机字体颜色
	
	//	m_pListCtrl->SetTextColor(RGB(255,0,255)); // 深粉红显示255 0 255 
...
LRESULT CGh0stView::OnAddToList(WPARAM wParam, LPARAM lParam)
{
	...
	try
	{
		...
		m_pListCtrl->SetItemText(i, 4, pszOS);
       ...
       //摄像头
		...
		strToolTipsText.Format("新的连接信息:\n主机名: %s\nIP  : %s\n操作系统: Windows %s", LoginInfo->HostName, IPAddress, strOS);
...
void CGh0stView::SendSelectCommand(PBYTE pData, UINT nSize)
{
	...
	int n = m_pListCtrl->GetItemCount();//获取行数
	int p = 0;
	for(int x = 0; x < n; x++)
	{
		if (m_pListCtrl->GetCheck(x) == TRUE)
		{
			p=p + 1;
		}			
	}
	...
	for(int i = 0; i < n; i++)//建立循环
	{
		if (m_pListCtrl->GetCheck(i) == TRUE)//复选框选中则发送
		{                
